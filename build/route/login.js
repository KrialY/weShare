...
module.exports = function () {
  ...
  router.post('/', (req, res) => {
    ...
    if (act === 'login') {
      db.query(`SELECT ID,username,password,uStatus from user_tab`, (err, data) => {
        let ok = true;
        if (err) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (item.username === username && item.password === password) {
              if(item.uStatus===1){
                res.send({'status': '6', 'info': '该用户已被封禁无法登陆'}).end();
                return;
              }
      ...
      db.query(`SELECT username,email from user_tab`, (err, data) => {
        ...
        if (err) {
          ...
          if (ok) {
            let avatar = 'avatar/defaultAvatar_register.jpg';
            db.query(`INSERT INTO user_tab (username,password,avatar,email) VALUE('${username}','${password}','${avatar}','${email}')`, (err) => {
              if (err) {
                console.error(err);
                res.status(500).send('database error').end();
              } else {
                db.query(`SELECT max(id) from user_tab`,(err,resData) => {
                  if (err) {
                    ...
                    res.send({'status': '0', 'info': '注册成功',"user_id":resData[0]['max(id)']}).end();
                  }
                });
              }
            });
