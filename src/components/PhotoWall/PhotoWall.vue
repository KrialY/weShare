<template>
  <section class="photowall" ref="wallWrapper" :style="`height:${h}px`">
    <img :src="`${PATH}1024-${src}`" @click="sendImgSrc">
    <p></p>
    <p class="download" @click.stop="triggleLogin">
      <a :href="download"></a>
    </p>
    <p class="contributor" :style="`background-image:url('${PATH}${avatar}');`">
      <a :href="profileDetail">{{contributor}}</a>
    </p>
...
export default {
  ...
  computed: {
    ...
    profileDetail () {
      if (parseInt(this.UPLOADID) === parseInt(this.loginUser)) {
        return `#/personcenter`;
      } else {
        return `#/usercenter?uploader=${this.UPLOADID}`;
      }
    },
    download () {
      if (this.loginUser === -1) {
        ...
        return `/api/download?fname=${this.src}`;
      }
    }
  },
  methods: {
