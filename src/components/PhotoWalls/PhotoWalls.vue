<template>
  <ul class="photowalls list_img row" ref="photoWrapper">
    <li v-for="(lis,i) in arr" :key="i">
      <div class="animateWrapper" ref="animateWrapper" v-for="(item,index) in arr[i]" :key="index">
        <PhotoWall :parent="parent" :loginUser="userId" :UPLOADID="item.UPLOADID" :ID="item.ID" :src="item.src" :likeType="item.like" :collectType="item.collect"  :h="parseInt(item.radioH)" @show-shadow="showShadow" :contributor="item.username"></PhotoWall>
      </div>
    </li>
  </ul>
</template>
...
export default {
  ...
  methods: {
    init () {
      ...
      this.arr = [[], [], []];
      this.nowNum = 0;
      let width = this.$refs.photoWrapper.children[0].offsetWidth - 10;
    ...
    add (_this) {
      ...
      if (_this.calH[utils.getMinHeight(_this.calH)] + utils.getTop(_this.$refs.photoWrapper) < scrollTop + document.documentElement.clientHeight) {
        if (onoff) {
          let addNum = _this.nowNum + _this.onceNum;
          addNum = addNum > _this.totalNum ? _this.totalNum : addNum;

          let width = this.$refs.photoWrapper.children[0].offsetWidth - 10;
          for (let i = _this.nowNum; i < addNum; i++) {
            let img = this.imgs[i % this.imgs.length];
            let radioH = img.height / img.width * width;
            let minIndex = utils.getMinHeight(this.calH);
            this.$set(img, 'radioH', radioH);
